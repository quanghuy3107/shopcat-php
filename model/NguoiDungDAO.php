<?php
    ...
    function insertNguoiDung($hovaten,$email,$diachinhanhang,$diachithanhtoan,$sodienthoai,$ngaysinh,$tendangnhap,$matkhau,$role_id){
        $sql = "INSERT INTO nguoidung(hovaten,email,diachinhanhang,diachithanhtoan,sodienthoai,ngaysinh,tendangnhap,matkhau,role_id) values ('{$hovaten}','{$email}','{$diachinhanhang}','{$diachithanhtoan}','{$sodienthoai}','{$ngaysinh}','{$tendangnhap}','{$matkhau}',{$role_id})";
        execute($sql);
    }

    function deleteNguoiDung($param){
        $sql = "DELETE FROM nguoidung WHERE nguoidung_id = {$param}";
        execute($sql);
    }

    function updateNguoiDung($id,$hovaten,$email,$diachinhanhang,$diachithanhtoan,$sodienthoai,$ngaysinh,$tendangnhap,$matkhau,$role_id){
        $sql = "UPDATE nguoidung SET hovaten = '{$hovaten}',email = '{$email}',diachinhanhang = '{$diachinhanhang}',diachithanhtoan = '{$diachithanhtoan}',sodienthoai = '{$sodienthoai}',ngaysinh = '{$ngaysinh}',tendangnhap = '{$tendangnhap}',matkhau = '{$matkhau}',role_id = {$role_id} WHERE nguoidung_id = {$id}";
        execute($sql);
    }
